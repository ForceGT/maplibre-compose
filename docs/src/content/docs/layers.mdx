---
title: Adding data to the map
description: Declare sources and layers as composables, style them with expressions, and anchor them in the layer stack.
---
...
As covered in [Styling the map](/maplibre-compose/styling/), the data displayed
on the map is defined by the style, which is provided as a URI to a JSON object.
However, this library provides a way to add additional data to the map at
runtime.
...
synchronous updates on Android and iOS. Web and Desktop ignore the option for
now. Under the hood, this maps to MapLibre Native's synchronous GeoJSON update
APIs such as Android's
[GeoJsonOptions.withSynchronousUpdate()][android-sync-update]:
...
<Code code={region(layers, "interaction")} lang="kotlin" />
