...
class FeatureStateTest {
  ...
  fun feature_state_is_merged_removed_and_read_back(): MapTestResult = runMapTest {
    if (!liveFeatureStateWorks) return@runMapTest
  ...
  private fun attachedPointSource(style: Style, featureId: String): GeoJsonSource {
    val source =
      GeoJsonSource(
        id = "points",
        data =
          GeoJsonData.Features(
            buildFeatureCollection<Geometry, JsonObject?> {
              addFeature(geometry = Point(Position(0.0, 0.0))) {
                ...
                if (number != null) setId(number) else setId(featureId)
              }
            }
          ),
        options = GeoJsonOptions(),
  ...
  private companion object {
    // maplibre-native-ffi 0.202608.0's JVM FFM writes FeatureStateSelector string views as
    // addresses rather than embedding mln_buffer_view, so native reads an empty source_id.
    // https://github.com/maplibre/maplibre-native-ffi/issues/623
    val liveFeatureStateWorks: Boolean
      get() = mapLibreFlavor != MapLibreFlavor.NATIVE
